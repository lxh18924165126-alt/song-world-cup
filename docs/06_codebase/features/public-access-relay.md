...
- `scripts/public-access-proxy.mjs`：固定上游 HTTP 反向代理、请求与响应流转发、生产健康检查、绝对重定向 origin 改写和错误响应。
...
2. Node 代理固定回源 `UPSTREAM_ORIGIN`，默认是生产 `song-world-cup.baituola-song-world-cup.workers.dev`；若 macOS 配置了可连接的 HTTP(S) 代理，Node 24 通过 `--use-env-proxy` 使用它访问生产站。
...
- `pnpm test:public-access`：覆盖正确/错误 `bootId`、跨城市通过门槛、真实首页标题、请求方法/查询/头/正文转发和重定向改写。
...
- `pnpm public:verify-cn`：真实大陆公网验证；2026-07-20 的健康和首页测量分别为 `2lAEEAf2qCL2lYBng00020nJE` 与 `2olnRTNlaP2t6CU3u00020nJE`，两项各有 5 个中国大陆城市成功。
